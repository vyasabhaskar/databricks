CREATE OR REFRESH STREAMING TABLE books_silver_dp;

CREATE FLOW books_flow
AS AUTO CDC INTO books_silver_dp
FROM stream(books_raw_dp)
KEYS (book_id)
SEQUENCE BY updated
COLUMNS * EXCEPT (updated)
STORED AS SCD TYPE 2;

CREATE OR REFRESH MATERIALIZED VIEW current_books_dp
AS SELECT book_id, title, author, price
   FROM books_silver_dp
   WHERE __END_AT IS NULL;

CREATE OR REFRESH STREAMING TABLE books_sales_dp(
  ...
  CONSTRAINT valid_date EXPECT(order_timestamp <= current_date() AND year(order_timestamp) >= 2020)
   ...
   INNER JOIN current_books_dp AS c
   ON b.book.book_id = c.book_id;

CREATE OR REFRESH MATERIALIZED VIEW authors_stats_dp
